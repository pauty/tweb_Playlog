...
/*
remove and recreate the welcome area to repeat fade animation
*/
...
function show_signup_form() {
	...
	intro_p.innerHTML = "<strong>Note: </strong>Please choose an user name from 6 to 20 characters long. Password must be at least 6 characters long and contain one letter and one digit";
	welcome_div.appendChild(intro_p);
...
function validate_passwords() {
	...
	if(/^(?=.*[a-zA-Z])(?=.*\d).{6,20}$/.test(password))
		$("password").setCustomValidity(""); //test passed, valid password
	else
	...
	if(password === r_password)
		$("repeat_password").setCustomValidity(""); //passwords match
	else
...
function validate_signup() {
	...
	/*test if informations provided respect the requested format*/
	email_value = $("email").value;
	...
	/*make ajax request to register new user*/
...
function handle_login(ajax){
	...
	if(res["success"] == 1){
		...
		/*some kind of error occurred, notify user*/
		var p = $("error_p");
		if(p != null)
		...
		if(res["success"] == 0){
			...
			p.innerHTML = "Sorry, an error occured. Please try again later";  //server side error
		}
		p.id = "error_p";
...
function handle_failure(){
	...
	if(p != null)
		$("welcome_area").removeChild(p);
	p = document.createElement("p");
	p.innerHTML = "Sorry, an error occured. Please try again later"; //error in making ajax request
	p.id = "error_p";
	$("welcome_area").insertBefore( p, $("welcome_area").childNodes[0] );
}